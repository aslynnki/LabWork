...
p = Popen(['pdb2gmx', '-f', pdb_filename], stdin=PIPE, stdout=PIPE)
p.communicate('1\n1\n')

i = 0
while i < len(bonds):
    if i == 8:
        bonds[i] = " " + str(i + 1) + "  " + str(i + 2) + "   2  BOND"
    if i <= 7:
        bonds[i] = " " + str(i + 1) + "   " + str(i + 2) + "   2  BOND"
    if i > 8:
        bonds[i] = str(i + 1) + "  " + str(i + 2) + "   2  BOND"
...
#pairs
i = 0
pairs = ["0" for x in range(len(atom_types_final) - 3)]
while i < len(pairs):
    if i <= 5:
        pairs[i] = " " + str(i + 1) + "   " + str(i + 4) + "   1"
    if i <= 8 and i >= 6:
        pairs[i] = " " + str(i + 1) + "  " + str(i + 4) + "   1"
    if i > 8:
        pairs[i] = str(i + 1) + "  " + str(i + 4) + "   1"
...
#angles
i = 0
angles = ["0" for x in range(len(atom_types_final) - 2)]
while i < len(angles):
    if i == 8:
        angles[i] = " " + str(i + 1) + " " + str(i + 2) + " " + str(i + 3) + "   2  ANGLE"
    if i == 7:
        angles[i] = " " + str(i + 1) + "  " + str(i + 2) + " " + str(i + 3) + "   2  ANGLE"
    if i <= 6:
        angles[i] = " " + str(i + 1) + "  " + str(i + 2) + "  " + str(i + 3) + "   2  ANGLE"
...
#dihedrals
i = 0
dihedrals = ["0" for x in range(len(atom_types_final) - 3)]
while i < len(dihedrals):
    if i == 8:
        dihedrals[i] = " " + str(i + 1) + " " + str(i + 2) + " " + str(i + 3) + " " + str(i + 4) + "   1  DIHEDRAL"
    if i == 7:
        dihedrals[i] = " " + str(i + 1) + "  " + str(i + 2) + " " + str(i + 3) + " " + str(i + 4) + "   1  DIHEDRAL"
    if i == 6:    
        dihedrals[i] = " " + str(i + 1) + "  " + str(i + 2) + "  " + str(i + 3) + " " + str(i + 4) + "   1  DIHEDRAL"
...
itp = open(itp_filename, 'w')
itp.write("[moleculetype]\n")
itp.write(";Name          nrexcl\n")
itp.write(three_letter_abbreviation)
itp.write("           3\n")
itp.write("\n[ atoms ]\n")
itp.write(";nr  type  resnr  resid  atom  cgnr   charge     mass\n")
i = 0
while i < len(atom_types_final):
    itp.write(nr[i])
    itp.write("   ")
    ...
    if i < 9:
        itp.write(" ")
    itp.write(str(atomic_charges[i]))
    itp.write("      ")
    itp.write(str(atomic_masses[i]))
    itp.write("\n")
    i = i + 1

itp.write("\n[ bonds ]\n")
itp.write("; ai  aj  fu   c0, c1, ...\n")
i = 0
while i < len(bonds):
    itp.write(bonds[i])
    itp.write(" ;  ")
    itp.write(atom_types_final[i])
    itp.write(" ")
    itp.write(atom_types_final[i+1])
    itp.write("\n")
    i = i + 1    
itp.write("\n[ pairs ]\n")
itp.write("; ai  aj  fu   c0, c1, ...\n")
...
while i < len(pairs):
    ...
    itp.write(atom_types_final[i])
    itp.write(" ")
    itp.write(atom_types_final[i+3])
    itp.write("\n")
    i = i + 1
itp.write("\n[ angles ]\n")
itp.write("; ai  aj  ak fu   c0, c1, ...\n")
i = 0
while i < len(angles):
    itp.write(angles[i])
    itp.write(" ;  ")
    ...
    itp.write(atom_types_final[i+1])
    itp.write(" ")
    itp.write(atom_types_final[i + 2])
    itp.write("\n")
    i = i + 1
itp.write("\n[ dihedrals ]\n")
itp.write("; ai  aj  ak al fu   c0, c1, m...\n")
i = 0
while i < len(dihedrals):
    itp.write(dihedrals[i])
    itp.write(" ; ")
    itp.write(atom_types_final[i])
    itp.write(" ")
    itp.write(atom_types_final[i + 1])
